...
| `SessionLocker` | Per-session mutex registry that serializes operations on the same session while allowing independent sessions to proceed in parallel. Instantiate with `&SessionLocker{}` and pass to consumers. `Unlock` panics if called without a matching `Lock`. |
